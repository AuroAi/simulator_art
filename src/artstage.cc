 ...
 *
 *  $Id: 9a835b55f53227a5100e40b2913ebbfd4f046644 $
 */
...
StageNode::mapName(const char *name, size_t robotID) {
	if (positionmodels.size() > 1) {
		static char buf[100];
		snprintf(buf, sizeof(buf), "/robot_%lu/%s", (long unsigned) robotID,
				name);
		return buf;
	} else
		return name;
...
StageNode::StageNode(int argc, char** argv, bool gui, const char* fname) {
	...
	for (size_t r = 0; r < numRobots; r++) {
		...
		vehicleModels_.push_back(
				new ArtVehicleModel(positionmodels[r], &tf, ""));
	}
}
...
int StageNode::SubscribeModels() {
	...
	for (size_t r = 0; r < this->positionmodels.size(); r++) {
		...
		laser_pubs_.push_back(
				n_.advertise<sensor_msgs::LaserScan>(mapName(FRONT_LASER, r),
						10));
...
void StageNode::WorldCallback() {
	...
	for (size_t r = 0; r < this->lasermodels.size(); r++) {
		...
		if (sensors.size() > 1)
			ROS_WARN(
					"ART Stage currently supports rangers with 1 sensor only.");
		...
		if (s.ranges.size()) {
			...
			this->laserMsgs[r].angle_increment = s.fov
					/ (double) (s.sample_count - 1);
			...
			for (unsigned int i = 0; i < s.ranges.size(); i++) {
				this->laserMsgs[r].ranges[i] =
						int(s.ranges[i]) == s.range.max ?
								(s.range.max - 1) : s.ranges[i];   //s.ranges[i]
