...
StageNode::ghfunc(Stg::Model* mod, StageNode* node)
{
  ...
  if (dynamic_cast<Stg::ModelPosition *>(mod))
  {
    ...
    ROS_INFO_STREAM("adding position model "<< node->positionmodels.size()+1);
  }
...
StageNode::SubscribeModels()
{
  ...
  for (size_t r = 0; r < this->positionmodels.size(); r++)
  {
    ...
    //getting the prefix
    char prefix[4];
    memcpy( prefix, this->positionmodels[r]->Token(), 3 );
    prefix[3] = '\0';
...
StageNode::~StageNode()
{    
  // if(pistionmodel.size()>=0) delete vehicle_model;
...
StageNode::WorldCallback()
{
  ...
  for (size_t r = 0; r < this->robotmodels_.size(); ++r)
  {
    if(this->robotmodels_[r]->type_veh)
    {
      vehicleModel->update(this->sim_time);
    }
    StageRobot const * robotmodel = this->robotmodels_[r];
